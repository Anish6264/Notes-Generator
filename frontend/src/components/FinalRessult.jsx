    ...
    <div className="mt-6 p-3 space-y-10 bg-white">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex gap-3">
          <button
            ...
            `}
          >
            {quickRevision ? "Exit Rivison Mode" : "Quick Revision(5 min )"}
          </button>

          <button
          onClick={()=>downloadPdf(result)}
          className="px-4 py-4 rounded-lg text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700">
            Download PDF
          </button>
        </div>
      </div>
      ...
      {result.diagram?.data && (
        <section>
          ...
          <p className="mt-3 text-xs text-gray-500 italic">
            If you need this diagram for further refrence or revision, you can
            save it by taking a schreenShot or by other method you like{" "}
          </p>
        </section>
      )}
      ...
      {result.charts?.length > 0 && 
        <section>
          ...
          <p className="mt-3 text-xs text-gray-500 italic">
            If you need this chart for further refrence or revision, you can
            save it by taking a schreenShot or by other method you like{" "}
          </p>
        </section>}

        {result.charts && result.charts.length===0 && (
            <p className="text-sm text-gray-400 italic"> charts are not relevent for this topic</p>
      )}
